...
public class RedisSessionManager extends AbstractSessionManager {
    ...
    @Resource(name = "addSessionScript")
    private DefaultRedisScript addSessionScript;
    ...
    public void addSession(SysUserDto sysUser, String group, String sessionId) {
        redisManager.execute(addSessionScript,redisManager.setKeys(getKey(sysUser),
                sysUser.getSessionId(),group,sessionId,String.valueOf(shiroProperties.getSessionTimeout())));
    }
...
//        redisManager.execute(delSessionScript,redisManager.setKeys(getKey(sysUser)));
        redisManager.del(RedisSessionDao.REDIS_SESSION_PREFIX+ Shiros.getSession().getId());
    }
}
